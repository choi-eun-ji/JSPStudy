...
<body>
	...
	<!-- fn:length()문자열의 길이를 계산하는 함수 -->
	<c:set var="genderno" value="${ fn:substring(jubun, 6, 7) }" />
	<!-- fn:substring()문자열에서 특정 인덱스의 문자열를 꺼내주는 함수 -->

	주민번호 : ${jubun}
	<br> 주민번호 문자열 길이 : ${len}
	<br> 성별을 나타내어주는 숫자 : ${genderno}
	<br>
	<br>
	...
	<c:if test="${ len ne 0 and len == 13}">

		<c:choose>
			<%-- <c:when>태그들을 감싸는 태그 --%>
			<c:when test="${ genderno eq '2' }">
				<!-- 자바의 if문. 이 if문이 해당되면 안에 있는 코드를 실행하고 탈출. 이 if문이 해당되지 않는다면 다음 when태그로 내려감. -->
            	1900년대생 여자입니다.
         	</c:when>

			<c:when test="${ genderno eq '4' }">
				<!-- 자바의 else if문. 이 if문이 해당되면 안에 있는 코드를 실행하고 탈출. 이 if문이 해당되지 않는다면 다음 when태그로 내려감. -->
            	2000년대생 여자입니다.
         	</c:when>

			<c:otherwise>
				<!-- 자바의 else문. 위의 모든 when태그가 해당되지 않았다면 이 안에 있는 코드를 실행하고 탈출. -->
            	남자입니다.
         	</c:otherwise>
		</c:choose>
