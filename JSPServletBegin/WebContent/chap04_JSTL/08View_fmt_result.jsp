...
<body>
   ...
   <c:if test="${ not empty pointArr1 }">
      <ul>
         <c:forEach var="point" items="${ pointArr1 }">
            <li>${ point }</li>
            <c:set var="sum" value="${ sum + point }" />   <%-- 자동형변환 --%>
         </c:forEach>
      </ul>
   ...
   <c:if test="${ not empty pointArr1 }">
      <ul style="list-style-type: circle;">
         <c:forEach var="point" items="${ pointArr1 }">
            ...
            <c:set var="sum" value="${ sum + pointInt }" />   <%-- 자동형변환 --%>
         </c:forEach>
      </ul>
      pointArr1 누적의 합계: ${ sum }
   </c:if>
   ...
   <h2>forEach를 사용해서 index 번호를 나타내기</h2>
   <c:if test="${ not empty pointArr1 }">
      <ul>
   ...
   <h2>forEach를 사용해서 순서번호를 나타내기</h2>
   <c:if test="${ not empty pointArr1 }">
      <ul>
         <c:forEach var="point" items="${ pointArr1 }" varStatus="status">
   ...
   <c:if test="${ not empty priceArr }">
      <ul>
         <c:forEach var="price" items="${ priceArr }">
            ...
            <c:set var="sum" value="${ sum + price }" />   <%-- 자동형변환 --%>
         </c:forEach>
      </ul>
